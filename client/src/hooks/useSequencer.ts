...
export function useSequencer(
  ...
  useEffect(() => {
    if (sampleCount > 0 && grid.length === 0) {
      ...
      if (savedGrid && savedGrid.grid.length > 0) {
        ...
        if (
          savedGrid.grid.length === sampleCount &&
          savedGrid.columns === columns
        ) {
          ...
          for (
            let r = 0;
            r < Math.min(sampleCount, savedGrid.grid.length);
            r++
          ) {
  ...
  const clearAllCells = useCallback(() => {
    if (grid.length > 0) {
      ...
      try {
        localStorage.removeItem("beatMaker_default");
      } catch (err) {
        console.error("Failed to remove from localStorage:", err);
      }

      // Create a completely new grid with all cells set to false
